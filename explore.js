(() => {
  ...
  const houseCredit = credit => /Burlington News/i.test(String(credit || '')) || /^(Graphic|Map|File photo|Courtesy photo)\b/i.test(String(credit || ''));
  const imageMarkup = (item, className) => item.image
    ? `<div class="${className}"><img src="${esc(item.image)}" alt="${esc(item.imageAlt || '')}" loading="lazy">${item.credit && !item.illustration && !houseCredit(item.credit) ? `<span class="image-credit">${esc(item.credit)}</span>` : ''}</div>`
