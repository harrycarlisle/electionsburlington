(() => {
  ...
  const assetUrl = value => {
    ...
    if (!raw || /^https?:\/\//.test(raw) || raw.startsWith('/')) return raw;
    return `/${raw}`;
  };
  const imageMarkup = (item, className) => item.image
    ? `<div class="${className}"><img src="${esc(assetUrl(item.image))}" alt="${esc(item.imageAlt || '')}" loading="lazy">${item.credit && !item.illustration && !/^Burlington News/i.test(item.credit) ? `<span class="image-credit">${esc(item.credit)}</span>` : ''}</div>`
