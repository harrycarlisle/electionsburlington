(() => {
  ...
  async function injectDailyBrief(){
    ...
    try{
      ...
      const meta=updated&&!Number.isNaN(updated.valueOf())?`Updated ${new Intl.DateTimeFormat('en-CA',{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'}).format(updated)}`:'Sources checked daily';
      section.innerHTML=`<div class="daily-brief-head"><div class="daily-brief-title-wrap"><span class="daily-brief-kicker">Burlington in 30 seconds</span><h2>What changed</h2></div><span class="daily-brief-meta">${meta}</span></div><div class="daily-brief-grid">${cards}</div><div class="daily-brief-footer"><p>Only the main point, why it matters and the original source.</p><a href="updates.html">See all updates →</a></div>`;
