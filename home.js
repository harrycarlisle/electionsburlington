...
(() => {
  ...
  function renderNewest(items,heroId){
    ...
    newestRail.hidden=false;newestRail.removeAttribute('aria-hidden');leadGrid?.classList.remove('is-hero-only');
  ...
  function renderPicks(items,readStats){
    ...
    const photoItems=items.filter(hasPhoto),fallback=items.filter(item=>!hasPhoto(item)),visible=[...photoItems,...fallback].slice(0,3);
    pickGrid.innerHTML=visible.map(item=>{const url=publicUrl(item.url),raw=item.image?(item.image.startsWith('/')?item.image:`/${item.image}`):CRIME_IMAGE,image=hasPhoto(item)?raw:CRIME_IMAGE,hook=displayDeck(item);return `<a class="pick-card" href="${esc(url)}"><div class="pick-image"><img src="${esc(image)}" alt="${esc(item.alt||item.headline)}" loading="lazy"></div><span class="kicker">${esc(categoryLabel(item))}</span><h3>${esc(displayHeadline(item))}</h3>${hook?`<p class="pick-hook">${esc(hook)}</p>`:''}</a>`}).join('')
  }
  function localReadStats(){try{return JSON.parse(localStorage.getItem('bn-article-read-counts')||'{}')}catch(_){return{}}}
  async function refresh(){
    try{
      const r=await fetch('/data/home-surface.json',{cache:'no-store'});if(!r.ok)return;const data=await r.json(),all=[...(data.feature||[]),...(data.latest||[]),...(data.rail||[])],feature=(data.feature||[]).find(hasPhoto)||all.find(hasPhoto);
      if(feature)renderLead(feature);
      const items=data.latest||data.items||all,newest=renderNewest(items,feature?.id),exclude=new Set([feature?.id,...newest.map(x=>x.id)].filter(Boolean)),picks=(data.popular||all).filter(x=>!exclude.has(x.id)).sort((a,b)=>popularityScore(b,localReadStats())-popularityScore(a,localReadStats()));renderPicks(picks,localReadStats())
    }catch(_){}
  }
