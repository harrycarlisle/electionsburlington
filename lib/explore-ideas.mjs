...
function inSeason(hours, now) {
  if (hours?.seasonStart && now.date < hours.seasonStart) return false;
  if (hours?.seasonEnd && now.date > hours.seasonEnd) return false;
  return true;
}

function withinWindow(hours, now, soonMinutes = 0) {
  ...
  if (!inSeason(hours, now)) return false;
  const open = parseHour(hours.open);
  const close = parseHour(hours.close);
  if (open == null || close == null) return false;
  const current = now.hour + (now.minute || 0) / 60;
  const soon = Number(soonMinutes) || 0;
  if (close <= open) return current >= open - soon / 60 || current < close;
  return current >= open - soon / 60 && current < close;
}

export function isOpenNow(idea, now) {
  const hours = idea?.hours;
  if (!hours) return null;
  const soon = hours.soonMinutes ?? 0;
  const regular = withinWindow(hours, now, soon);
  const evening = hours.evening ? withinWindow(hours.evening, now, 0) : false;
  if (regular || evening) return true;
  const days = hours.days || hours.openDays || [];
  const eveningDays = hours.evening?.days || [];
  if ((days.length || eveningDays.length) && parseHour(hours.open) != null) return false;
  return null;
}
...
export function isActive(idea, now) {
  ...
  if (idea?.hours && !inSeason(idea.hours, now) && !withinWindow(idea.hours.evening, now, 0)) return false;
  return true;
}
