...
export function journeySortMinutes(journey) {
  ...
  return minutes + (journey?.nextServiceDay && minutes < 1440 ? 1440 : 0);
}
...
export function buildGoModel(data, now = new Date()) {
  ...
  // The homepage utility is for people leaving Burlington, not trains elsewhere
  // on Lakeshore West. Compare the next Burlington-origin departure eastbound
  // to Union and westbound to West Harbour, then show whichever leaves first.
  const burlingtonRoutes = [
    findRoute(data, 'BU', 'UN'),
    findRoute(data, 'BU', 'WR')
  ].filter(Boolean);
  const candidates = burlingtonRoutes.flatMap(route =>
    upcomingJourneys(route, clock.nowMin).slice(0, 1).map(journey => ({ route, journey }))
  ).sort((a, b) => (journeySortMinutes(a.journey) ?? 9999) - (journeySortMinutes(b.journey) ?? 9999));

  const chosen = candidates[0] || null;
  const chosenRoute = chosen?.route || burlingtonRoutes[0] || null;
  const journey = chosen?.journey || null;
  ...
  const origin = chosenRoute?.origin?.label || 'Burlington';
  const dest = chosenRoute?.destination?.label || 'Union';
