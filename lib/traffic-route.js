...
const MAX_CAMERAS = 6;
const WESTBOUND_VIEWS = new Set([10, 2560, 2420, 588]);
const EASTBOUND_VIEWS = new Set([598, 599, 583, 603, 605]);
...
export function cameraConflictsRoute(cam, routeId) {
  ...
  if (east) {
    ...
    if (WESTBOUND_VIEWS.has(Number(cam.viewId))) return true;
    if (!EASTBOUND_VIEWS.has(Number(cam.viewId))) return true;
    return false;
  }
