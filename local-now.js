...
(() => {
  ...
  function todayModel(explore) {
    ...
    const items = (explore?.items || explore?.events || [])
      .filter(item => !item?.end || new Date(item.end).getTime() >= now)
      .sort((a, b) => new Date(a.start || 0) - new Date(b.start || 0));
    const item = items[0];
    return item ? {
      title: item.title || item.name,
      relative: item.dateLabel || item.relative || '',
      hours: item.venue || item.hours || '',
      url: item.url || '/explore/'
    } : null;
  }
  ...
  function bindCard(target) {
    ...
    card.addEventListener('touchend', event => {
      ...
      if (swiping && Math.abs(dx) >= SWIPE_PX) stepMode(dx < 0 ? 1 : -1);
    });
  ...
  function render(payload) {
    ...
    selectedMode = selectedMode || 'driving';
