...
(() => {
  ...
  document.addEventListener('DOMContentLoaded',()=>{buildPublicationHome();run();const body=document.body;if(!body||body.dataset.bundleWatch==='1')return;body.dataset.bundleWatch='1';let queued=false;new MutationObserver(()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;run()})}).observe(body,{childList:true,subtree:false})},{once:true});
})();
